...
const StudentQuiz = () => {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:5085/api/quizzes/${quizId}`, {
            ...
            .catch(err => {
                addToast('Failed to load quiz metadata', 'error');
                setLoading(false);
            });
    }, [quizId]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(selectedAnswers).length === 0) {
            return addToast('Please select at least one answer before submitting', 'error');
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                addToast(`Quiz submitted! You scored ${data.score}/${data.total}`, 'success');
            } else {
